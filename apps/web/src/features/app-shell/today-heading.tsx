...
export function TodayHeading() {
  ...
  useEffect(() => {
    if (!now) return;
    fetch('http://127.0.0.1:7869/ingest/c694b7eb-dcc2-4100-9c19-d4aca06d483e',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'a506d6'},body:JSON.stringify({sessionId:'a506d6',runId:'post-fix',hypothesisId:'R',location:'today-heading.tsx',message:'header date timezone',data:{locale,timeZone,weekday,date,dateTime,utc:now.toISOString()},timestamp:Date.now()})}).catch(()=>{});
